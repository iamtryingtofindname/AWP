...
function helios:repairKey()
    local keyPath = helios:getDirectory(helios.Main,helios.Key)
...
function helios:setKey(key)
    local keyPath = helios:getDirectory(helios.Main,helios.Key)
    local old = helios:repairKey()
...
function helios:Init(placeId,directory,metadataNow,useKeySystem)
    ...
    return decoded,old_metadata,key
end
